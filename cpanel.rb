...
module CpanelAPI
  ...
  def json_url(module_name,func_name,options='')
  ...
  module CpanelDbUtil
    ...
    def list_dbs
      url = json_url(@db_file_name[:api][@db_type],'listdbs')
    ...
    def list_db_users
      url = json_url(@db_file_name[:api][@db_type],'listusers')
    ...
    def list_db_priv(db)
      ...
      url = json_url(@db_file_name[:api][@db_type],'listusersindb',"db=#{db}")
  ...
  module CpanelDomainUtil
    ...
    def list_domains
      ...
      url = json_url('DomainLookup','getbasedomains')
      
      get(url).inject([]) { |m,x| x['domain']}
    end
  
    def get_doc_root(domain)
      url = json_url('DomainLookup','getdocroot',"domain=#{domain}")
      data = get(url)
    ...
    def list_park_domains
      url = json_url('Park','listparkeddomains')
      
      get(url).inject([]) { |m, x| m << x['domain'] }
    end
  
    def list_addon_domains
      url = json_url('AddonDomain','listaddondomains')
      
      get(url).inject([]) { |m,x| m << x['domain'] }
    end
  
    def list_sub_domains
      url = json_url('SubDomain','listsubdomains')
    ...
    def park_domain(domain)
      ...
      unless is_park_domain?(domain)
        url = json_url('Park','park',"domain=#{domain}")
        data = get(url)
        if data[0]['result'] == 1
    ...
    def add_addon_domain(domain,dir='',ftp_user='',pass=random)
      ...
      unless is_addon_domain?(domain)
        url = json_url('AddonDomain','addaddondomain',"newdomain=#{domain}&dir=#{dir}&subdomain=#{ftp_user}&pass=#{pass}")
        data = get(url)
        if data[0]['result'] == 1
    ...
    def add_sub_domain(domain,rootdomain,dir='')
      ...
      unless is_sub_domain?(full_sub_domain)
        url = json_url('SubDomain','addsubdomain',"domain=#{domain}&rootdomain=#{rootdomain}&dir=#{dir}")
        data = get(url)
        if data[0]['result'] == 1
    ...
    def del_park_domain(domain)
      domain = scan_domain_name(domain)
      url = json_url('Park','unpark',"domain=#{domain}")
      data = get(url)
      if data[0]['result'] == 1
    ...
    def del_addon_domain(domain)
      domain = scan_domain_name(domain)
      url = json_url('AddonDomain','deladdondomain',"domain=#{domain}")
      data = get(url)
      if data[0]['result'] == 1
        puts data[0]['reason']
        return_msg(true,data[0]['reason'],:domain_name => "#{domain}")
    ...
    def del_sub_domain(domain)
      domain = scan_domain_name(domain)
      url = json_url('SubDomain','delsubdomain',"domain=#{domain}")
      data = get(url)
